...
val minmax_xy : t -> num * num * num * num
...
module Regular: sig
  ...
  type t = private {
    ...
    snd : Point.t;
    len : num;
    apothem : num; (** Distance between the center and the middle of one of the edges *)
    edges : int;
  }
  ...
  val to_randomized_polygon : ?minp:int -> ?prob:num -> t -> polygon
  val translate : num -> num -> t -> t
  val transform : Affine.t -> t -> t
