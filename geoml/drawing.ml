...
let point2pixel p = Point.(Arith.to_int p.x, Arith.to_int p.y)
...
let screen () =
 let sx = Arith.of_int (size_x ())
 and sy = Arith.of_int (size_y ()) in
...
let draw_circle ?(lw=1) c col =
  ...
  draw_circle x y (c.radius |> Arith.to_int)
...
let fill_circle ?(lw=1) c col =
  ...
  set_line_width lw;
  let x,y = point2pixel c.center in
  fill_circle x y (c.radius |> Arith.to_int)
...
let draw_ellipse ?(lw=1) e col =
  ...
  draw_ellipse x y (Arith.to_int (big_axis e)) (Arith.to_int (small_axis e))
